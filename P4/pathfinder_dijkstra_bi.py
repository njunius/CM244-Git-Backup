# Navmesh Pathfinder Bidirectional Dijkstra's Algorithm
# Nick Junius
...
def find_path(source_point, dest_point, mesh):
    ...
    path, visited_nodes = dijkstra_shortest_path(src_box, source_point, dst_box, dest_point, mesh, dist_point_in_box)
    
    return(path, visited_nodes)
# bidirectional dijkstra's 
def dijkstra_shortest_path(src_box, src_point, dst_box, dst_point, graph, adj):
    ...
    while frontier:
        ...
        for next in graph["adj"][current]:
            ...
            # forward section
            if goal_point == dst_point:
                new_cost, new_point = adj(forward_detail_points[current], next, forward_cost_so_far[current], goal_point)
                if next not in forward_cost_so_far or new_cost < forward_cost_so_far[next]:
                    ...
                    heappush(frontier, (new_cost, next, goal_point))
                    visited_nodes.append(next)
                    forward_detail_points[next] = new_point
                    
            # backwards section
            if goal_point == src_point:
                new_cost, new_point = adj(backwards_detail_points[current], next, backwards_cost_so_far[current], goal_point)
