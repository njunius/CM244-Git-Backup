...
class World:
  ...
  def populate(self, specification, brain_classes):
    ...
    for i in range(specification.get('obstacles',0)):
      o = Obstacle(self)
      o.radius = 5+150*random.random()*random.random()
...
from brains import MantisBrain, SlugBrain
...
master = tkinter.Tk()
master.title("Slugs and Mantises")

world = World(CANVAS_WIDTH, CANVAS_WIDTH)
world.populate(world_specification, brain_classes)
